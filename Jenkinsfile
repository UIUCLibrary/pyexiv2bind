                            ...
                            script {
                                ...
                                node("Windows") {
                                    withCredentials([usernamePassword(credentialsId: 'DS_devpi', usernameVariable: 'DEVPI_USERNAME', passwordVariable: 'DEVPI_PASSWORD')]) {
                                        ...
                                        bat "${tool 'Python3.6.3_Win64'} -m venv venv"
                                        unstash "tests"
                                        bat """ ${tool 'Python3.6.3_Win64'} -m pip install -Iv ${name}==${version} -i /${DEVPI_USERNAME}/${env.BRANCH_NAME}_staging --no-cache-dir --no-binary :all:
                                            call venv\\Scripts\\activate.bat
                            ...
                            script {
                                ...
                                node("Windows") {
                                    withCredentials([usernamePassword(credentialsId: 'DS_devpi', usernameVariable: 'DEVPI_USERNAME', passwordVariable: 'DEVPI_PASSWORD')]) {
                                        ...
                                        bat "${tool 'Python3.6.3_Win64'} -m venv venv"
                                        unstash "tests"
                                        bat """ ${tool 'Python3.6.3_Win64'} -m pip install -Iv ${name}==${version} -i /${DEVPI_USERNAME}/${env.BRANCH_NAME}_staging --no-cache-dir  --only-binary bdist_wheel
                                            call venv\\Scripts\\activate.bat
                                            ${tool 'Python3.6.3_Win64'} -m pytest"""
                                    }
                                }
